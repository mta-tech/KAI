...
│  └──────────┴──────────┴──────────┴──────────┴──────────┘      │
│  ┌──────────┬──────────┬──────────┬──────────┬──────────┐      │
│  │  Skill   │  Visual  │ Context  │Business  │   RAG    │Context     │      │
│  │ Service  │ Service  │ Service  │Glossary  │ Service  │ Platform   │      │
│  └──────────┴──────────┴──────────┴──────────┴──────────┘      │
└────────────────┬────────────────────────────────────────────────┘
                 │
                 ▼
...
- `ContextAssetService` - Context asset lifecycle (create, promote, deprecate)
- `BenchmarkService` - Benchmark execution and results tracking
- `TelemetryService` - Reuse metrics and KPI tracking
...
### Context Platform Module (`app/modules/context_platform/`)

**Purpose**: Manage reusable domain knowledge artifacts and quality evaluation.

The Context Platform provides:
- **Context Asset Lifecycle**: Create, version, and manage domain artifacts (table descriptions, glossaries, instructions, skills)
- **Benchmark System**: Evaluate asset quality through test suites and cases
- **Feedback Collection**: Gather user feedback for continuous improvement
- **Telemetry**: Track reuse metrics and ROI

**Structure**:
```
app/modules/context_platform/
├── models/
│   ├── asset.py          # ContextAsset, ContextAssetVersion, ContextAssetTag
│   ├── benchmark.py       # BenchmarkCase, BenchmarkSuite, BenchmarkRun, BenchmarkResult
│   └── feedback.py        # Feedback, FeedbackRequest, FeedbackResponse
├── repositories/
│   ├── asset_repository.py      # CRUD operations for assets
│   └── benchmark_repository.py  # CRUD operations for benchmarks
├── services/
│   ├── asset_service.py      # Asset lifecycle management (create, promote, deprecate)
│   ├── benchmark_service.py   # Benchmark execution and results
│   └── telemetry_service.py   # Reuse metrics and KPI tracking
└── api/
...
**Key Models**:
- `ContextAsset`: Reusable domain knowledge with versioning
  - `asset_type`: table_description, glossary, instruction, skill
  - `lifecycle_state`: draft, verified, published, deprecated
  - `canonical_key`: Unique key within database and type
- `BenchmarkCase`: Test case for evaluating asset quality
- `BenchmarkRun`: Execution of a suite with aggregated results
- `Feedback`: User validation and improvement suggestions
...
kai context create --db mydb --type instruction --key my_key --name "My Asset"
kai context update asset_id [--name "New Name"]
kai context delete db_id instruction my_key
kai context search --db mydb --query "sales analysis"
kai context promote asset_id verified --by "User" [--note "Reason"]
kai context deprecate asset_id --by "User" --reason "Replaced"
kai context tags [--category domain]

# Benchmarks
kai benchmark list [--db mydb]
kai benchmark info suite_id
kai benchmark run suite_id --db mydb [--tags smoke,critical] [--export results.json]
...
### Example: Context Asset Lifecycle

```
1. User: "Create a verified SQL pattern for customer lifetime value"
        ↓
2. CLI: kai context create --db mydb --type verified_sql --key clv_sql ...
        ↓
3. ContextAssetService.create_asset()
        ↓
4. Asset created in DRAFT state
        ↓
5. User: kai context promote asset_id verified --by "Expert"
        ↓
6. ContextAssetService.promote_to_verified()
        ↓
7. Asset state changes to VERIFIED
        ↓
8. TelemetryService.track_asset_promotion()
        ↓
9. Asset available for reuse in missions
```

### Example: Benchmark Execution

```
1. User: "Run quality benchmarks on new context assets"
        ↓
2. CLI: kai benchmark run suite_123 --db mydb
        ↓
3. BenchmarkService.create_run()
        ↓
4. For each test case:
        a. Load case definition
        b. Execute with context assets
        c. Compare actual vs expected
        d. Calculate similarity score
        e. Store BenchmarkResult
        ↓
5. BenchmarkService.complete_run()
        ↓
6. Aggregate metrics (pass rate, weighted score, severity breakdown)
        ↓
7. TelemetryService.track_benchmark_completion()
        ↓
8. Return results with export option (JSON/JUnit)
```
...
- **Typesense** - Vector search and document store
  - Collections: `sessions`, `sql_generations`, `prompts`, `glossaries`, `instructions`, `skills`, `verified_sql`, `dashboards`, `context_assets`, `benchmark_suites`, `benchmark_cases`, `benchmark_runs`, `benchmark_results`, `feedback`, `telemetry_events`
...
- [Context Platform Documentation](docs/context-platform/)
