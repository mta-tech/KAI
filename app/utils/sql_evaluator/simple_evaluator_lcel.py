"""Simple Evaluator using LCEL (LangChain Expression Language) - replacement for deprecated LLMChain."""
...
class SimpleEvaluatorLCEL(Evaluator):
    """Simple Evaluator using LCEL pattern instead of deprecated LLMChain."""
    ...
    def create_sql_results(self, result: Any) -> list:
        """Convert SQL results to serializable format."""
        rows = []
        if result:
            for row in result:
                modified_row = {}
    ...
    def evaluate(
        ...
        """Evaluate SQL query using LCEL chain."""
        ...
        logger.info(
            f"(Simple evaluator LCEL) Generating score for the question/sql pair: "
            f"{str(user_prompt.text)}/ {str(sql_generation.sql)}"
        )
        ...
        # Build the LCEL chain: prompt | llm | output_parser
        ...
        try:
            ...
            logger.info(
                f"(Simple evaluator LCEL) error while parsing the SQL query: {str(e)}. Returning score 0"
            )
            return Evaluation(
        ...
        if sql_generation.status == "INVALID":
            logger.info(
                f"(Simple evaluator LCEL) SQL query: {sql} is not valid. Returning score 0"
            )
            return Evaluation(
                question_id=user_prompt.id, answer_id=sql_generation.id, score=0
            )
        ...
        if not rows:
            logger.info(
                f"(Simple evaluator LCEL) SQL query: {sql} returned no results. max confidence is 70"
            )
            max_confidence = 70
        ...
        logger.info(f"(Simple evaluator LCEL) answer of the evaluator: {answer}")
        score = self.answer_parser(answer=answer) / 100
        logger.info(f"(Simple evaluator LCEL) score of the evaluator: {str(score)}")
